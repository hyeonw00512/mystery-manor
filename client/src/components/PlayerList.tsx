...
export function PlayerList() {
  ...
  return <section className="panel player-panel"><div className="section-heading"><div><span className="eyebrow">참가 현황</span><h2>조사관 {room?.players.length}/{room?.rules.maxPlayers}</h2></div><span className="step-count">02</span></div><div className="player-list">{room?.players.map((player, index) => {
    ...
    return <article className={`player-row ${!player.connected ? "disconnected" : ""}`} key={player.playerId} style={{ "--character-color": character?.color ?? "#343b45" } as React.CSSProperties}>
      <span className="player-number">{String(index + 1).padStart(2, "0")}</span><span className="mini-avatar">{character?.initials ?? "?"}</span>
